...
public class Board extends JPanel implements ActionListener {
    ...
    private int curX = 0;
    private int curY = 0;
    private ArrayList<Integer> filledBlocks;
    ...
    private void removeFullLines() {
        ...
        int numFullLines = 0;
        var totalFilledBlocks = filledBlocks.size();
        ...
        if (numFullLines > 0) {
            ...
            for (int i = 0; i < totalFilledBlocks; ++i) {

                int idx = filledBlocks.get(i);

                if (idx < BOARD_HEIGHT) {

                    for (int j = idx; j < BOARD_HEIGHT - 1; ++j) {
                        filledBlocks.set(i, j);
                    }
                }
            }

            if (numLinesRemoved >= 10) {
                timer.setDelay(DELAY - numLinesRemoved * 10);
            }

            clearBoard();

            for (int i = 0; i < totalFilledBlocks - numFullLines; ++i) {
                board[filledBlocks.get(i)] = Shape.Tetrominoes.ShapeCount;
            }

            filledBlocks.subList(0, totalFilledBlocks - numFullLines).clear();
